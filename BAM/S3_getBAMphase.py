...
for dtname in datasets:
    ...
    Datestamp = pd.date_range(
        start=f"{start_year}-01-01",
        end=f"{end_year}-12-31"
    )
    Date0 = pd.DataFrame({'date': Datestamp})
    ...
    for k in ['SH','NH']:
        ...
        # read the BAM index
        BI = np.load(f'{INDIR}/{dtname}_{k}_BAM_index_total_no_leap.npy')                             ### This is full BAM index from 1979 to 2021, 15695 in total, no leap years ###
        ...
        np.save(f'{INDIR}/{dtname}_{k}_BAM_index_total_with_leap.npy', BI)   ### This is full BAM index from 1979 to 2021, 15695 in total, with leap years ###
