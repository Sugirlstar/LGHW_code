...
Duration = 5*4 # 5 days, 6-hourly data, 20 timesteps
...
for d in np.arange(nday-1):
    ...
    for i in np.arange(len(lon_d[d])):
        ...
        if day+1 >= Duration and n_large_wave>=5*4 and n_north>=5*4:
...
#%%
with open("/scratch/bell/hu1029/LGHW/Blocking_date", "wb") as fp:
    pickle.dump(Blocking_date, fp)
with open("/scratch/bell/hu1029/LGHW/Blocking_lon", "wb") as fp:
    pickle.dump(Blocking_lon, fp)  
with open("/scratch/bell/hu1029/LGHW/Blocking_lat", "wb") as fp:
    pickle.dump(Blocking_lat, fp) 
with open("/scratch/bell/hu1029/LGHW/Blocking_lon_wide", "wb") as fp:
    pickle.dump(Blocking_lon_wide, fp)
with open("/scratch/bell/hu1029/LGHW/Blocking_area", "wb") as fp:
    pickle.dump(Blocking_area, fp)
with open("/scratch/bell/hu1029/LGHW/Blocking_label", "wb") as fp:
    pickle.dump(Blocking_label, fp)
np.save("/scratch/bell/hu1029/LGHW/B_freq2.npy", B_freq2)
np.save("/scratch/bell/hu1029/LGHW/B_freq.npy",B_freq)
...
plt.ylabel('Latitude')
plt.savefig('blockingFreq_LongitudeWideby4.png')  
plt.show()
plt.close()
