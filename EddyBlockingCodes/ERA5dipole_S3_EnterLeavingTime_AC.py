...
for typeid in [1,2,3]:
    ...
    BlockIndexSec2 = np.load(f'/scratch/bell/hu1029/LGHW/ERA5dipole_TrackBlockingType{typeid}_IndexSector2_1979_2021_AC.npy') # the related blocking event id, -1 represent no blocking related
    ...
    print('Number of AC tracks that has ever enter into the Sec2 region:')
    print(n)
    print('Number of AC tracks that has ever interact with blocking:')
    ...
    print(Sec2BlockPersis)
    print(tknumlist)

    corr, p_value = pearsonr(tknumlist_clean, Sec2BlockPersis_clean)
    corr, p_value = pearsonr(tknumlist, Sec2BlockPersis)

    print(f'correlation coefficient between the blocking type{typeid} persistence and the related AC track numbers:')
    ...
    # plt.legend()
    # plt.show()
    # plt.savefig(f'ERA5dipole_AC_BlockingEventEnterLeaveTimeDistribution_blkType{typeid}.png')
    # plt.close()

    # # group by the type of the tracks
    # InterTypeSec2 = np.load(f'/scratch/bell/hu1029/LGHW/ERA5dipole_BlockingType{typeid}_InteractionTypeSec2_1979_2021_AC.npy')
    # Throughid = np.where(InterTypeSec2 == 'T')[0]
    # Absorbedid = np.where(InterTypeSec2 == 'A')[0]
    # Edgeid = np.where(InterTypeSec2 == 'E')[0]
    # print(f'number of AC through: {len(Throughid)}')
    # print(f'number of AC absorbed: {len(Absorbedid)}')
    # print(f'number of AC edge: {len(Edgeid)}')
