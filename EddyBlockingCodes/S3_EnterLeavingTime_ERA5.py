...
for dtname in datasets:
    ...
    trackreflat = TRACK_latrefFile[dtname]
    trackreflon = TRACK_lonrefFile[dtname]
    
    # lat lon for TRACKS (F128)
    lon = np.load(trackreflon)
    lat = np.load(trackreflat)
    # time management
    ...
    for rgname in regions:
        ...
        lonBLK = np.load(lonfile)
        latBLK = np.load(latfile) # descending order
        ...
        for typeid in [1,2,3]:
            for cyc in cycTypes:
                for ssi,ss in enumerate(seasons):
                    ...
                    eventPersistence = np.load(f'{OUTDIR}/{dtname}_BlockingEventPersistence_Type{typeid}_{rgname}_{ss}.npy') # load the event persistence
                    # the 1st day's date
                    ...
                    BlockingEIndexSec2 = np.load(f'{OUTDIR}/{dtname}_SD_BlockingFlagmaskClusters_Type{typeid}_{rgname}_{ss}.npy')
                    ...
                    # make dir ./{dtname}_SD_enterleave if not exist
                    if not os.path.exists(f'./{dtname}_SD_enterleave'):
                        os.makedirs(f'./{dtname}_SD_enterleave')
                    
                    with open(f"./{dtname}_SD_enterleave/{dtname}_BlockingBasedProbability.txt", "a") as f:  
                    ...
                    with open(f"./{dtname}_SD_enterleave/{dtname}_EddyBasedProbability.txt", "a") as f:  
                    ...
                    np.save(f'{OUTDIR}/{dtname}_EnterTimePointr2Blk1stDay_type{typeid}_{cyc}_{rgname}_{ss}.npy', entertime)
                    np.save(f'{OUTDIR}/{dtname}_LeaveTimePointr2Blk1stDay_type{typeid}_{cyc}_{rgname}_{ss}.npy', leavetime)

                    np.save(f'{OUTDIR}/{dtname}_intopercentList_type{typeid}_{cyc}_{rgname}_{ss}.npy', np.array(intopercentList))
                    np.save(f'{OUTDIR}/{dtname}_leavepercentList_type{typeid}_{cyc}_{rgname}_{ss}.npy', np.array(leavepercentList))
                    np.save(f'{OUTDIR}/{dtname}_interactingduration_type{typeid}_{cyc}_{rgname}_{ss}.npy', np.array(interactingduration))
    ...
    for typeid in [1,2,3]:
        for cyc in cycTypes:
            for rgname in regions:
                ...
                print(f'statistics: Blocking type{typeid} - {cyc} - {rgname} - {ss}')
                intopercentList = np.load(f'{OUTDIR}/{dtname}_intopercentList_type{typeid}_{cyc}_{rgname}_{ss}.npy')
                interactingduration = np.load(f'{OUTDIR}/{dtname}_interactingduration_type{typeid}_{cyc}_{rgname}_{ss}.npy')
