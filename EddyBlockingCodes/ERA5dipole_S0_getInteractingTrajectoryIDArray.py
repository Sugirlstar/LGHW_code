...
for typeid in [1, 2, 3]:
    ...
    np.save(f'/scratch/bell/hu1029/LGHW/ACtrackPoints_TrackIDarray_inBlkType{typeid}.npy', trackPoints_array)
    ...
    for t, la, lo, tid in zip(time_indices, lat_indices, lon_indices, track_id_list):
        trackPoints_array[t, la, lo] = tid
    np.save(f'/scratch/bell/hu1029/LGHW/CCtrackPoints_TrackIDarray_inBlkType{typeid}.npy', trackPoints_array)

# check the results
test = np.nansum(trackPoints_array,axis=(1,2))
ki = np.where(test>0)[0][0]
print(ki)
print(np.unique(trackPoints_array[ki,:,:]))
k2 = np.where(test>0)[0][-1]
print(k2)
print(np.unique(trackPoints_array[k2,:,:]))
print('done')
