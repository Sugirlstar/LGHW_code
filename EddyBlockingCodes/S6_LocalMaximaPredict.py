...
for rgname in regions:
    ...
    #     if day % 500 == 0:
    #         print('day', day, ' - label filtered', flush=True)
    ...
    for typeid in [1, 2, 3]:
        ...
        for k,dayi in enumerate(firstblkIndex):
            ...
            center0 = np.unravel_index(np.argmax(LWA_td[dayi,:,:] * mask0, axis=None), LWA_td[dayi,:,:].shape)
            centerlatindex = center0[0]  # the latitude index of the center
            centerlonindex = center0[1]  # the longitude index of the center
            LocalMaxima = np.nanmax(LWA_td[dayi,:,:] * mask0)
