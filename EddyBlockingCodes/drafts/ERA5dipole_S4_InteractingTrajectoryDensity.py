...
for typeid in [1, 2, 3]:
    ...
    # get the interact id
    InterTypeSec2CC = np.load(f'/scratch/bell/hu1029/LGHW/ERA5dipole_TrackBlockingType{typeid}_IndexSector2_1979_2021_CC.npy')
    InterTypeSec2AC = np.load(f'/scratch/bell/hu1029/LGHW/ERA5dipole_TrackBlockingType{typeid}_IndexSector2_1979_2021_AC.npy')
    interidCC = np.where(InterTypeSec2CC != -1)[0]
    interidAC = np.where(InterTypeSec2AC != -1)[0]

    # AC tracks
    with open('/scratch/bell/hu1029/LGHW/ACZanom_allyearTracks.pkl', 'rb') as file:
        track_data = pickle.load(file)
    track_data = [track_data[i] for i in interidAC]
    ...
    print(f"AC track intersection with Blocking type {typeid} total number:", len(time_list))
    ...
    np.save(f'/scratch/bell/hu1029/LGHW/ACtrackPoints_array_inBlkType{typeid}.npy', trackPoints_array)
    ...
    plt.show()
    plt.tight_layout()
    plt.savefig(f'ERA5dipole_AC_pointFrequency_inBlkType{typeid}.png')
    plt.close()
    ...
    with open('/scratch/bell/hu1029/LGHW/CCZanom_allyearTracks.pkl', 'rb') as file:
        track_data = pickle.load(file)
    track_data = [track_data[i] for i in interidCC]
    ...
    print(f"CC track intersection with Blocking type {typeid} total number:", len(time_list))
    # make an array representing the trackpoint density
    trackPoints_array = np.zeros((len(datetime_array), len(latNH), len(lon)))
    time_indices = np.searchsorted(timeiarr, time_list)
    lat_indices = np.array([findClosest(x, latNH) for x in x_list])
    ...
    np.save(f'/scratch/bell/hu1029/LGHW/CCtrackPoints_array_inBlkType{typeid}.npy', trackPoints_array)
    trackPoints_frequency = np.nansum(trackPoints_array, axis=0) 
    ...
    plt.show()
    plt.tight_layout()
    plt.savefig(f'ERA5dipole_CC_pointFrequency_inBlkType{typeid}.png')
    plt.close()
