...
for typeid in [3,1]:
    ...
    BlockIndexSec2 = np.load(f'/scratch/bell/hu1029/LGHW/Dipole_Merra2_TrackBlockingType{typeid}_IndexSector2_1979_2021_AC.npy') # the related blocking event id, -1 represent no blocking related
    ...
    InterTypeSec2 = np.load(f'/scratch/bell/hu1029/LGHW/Dipole_Merra2_BlockingType{typeid}_InteractionTypeSec2_1979_2021_AC.npy')
    ...
    def getCompFig(fname, extendlen, targetTrackIndexCommon, BlockIndexSec2, BlockingEIndexSec2):
        ...
        cbar.set_label('LWA')    
        plt.show()
        plt.savefig(f'Dipole_AC_getCompositeColorBarSec2_{fname}_BlkType{typeid}.png')
        plt.close()         
        ...
        for i in range(len(TargetperiodTrackList)):
            ...
            plt.show()
            plt.savefig(f'Dipole_AC_AnimationCompositePoints_{fname}_BlkType{typeid}_{i}.png')
            plt.close()
        ...
        frames = sorted([
            ...
            if f.startswith(f'Dipole_AC_AnimationCompositePoints_{fname}_BlkType{typeid}_') and f.endswith('.png')
        ], key=lambda f: int(f.split('_')[-1].split('.')[0]))
        print(frames)
        # get all the images
        gif_filename = f"Dipole_AC_Animation_compositePoints_{fname}_BlkType{typeid}.gif"
