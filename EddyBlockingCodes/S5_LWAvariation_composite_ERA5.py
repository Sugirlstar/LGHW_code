...
for dtname in datasets:
    ...
    lon = np.load(lonfile)
    lat = np.load(latfile) # descending order 90~-90
    ...
    for typeid in [1,2,3]: 
        for cyc in cycTypes:
            for rgname in regions:            
                    ...
                    blockingEidArr = np.load(f'{INDIR}/{dtname}_SD_BlockingClustersEventID_Type{typeid}_{rgname}_{ss}.npy') # the blocking event index array
                    ...
                    with open(f"{INDIR}/{dtname}_BlockEventDailyLWAList_{yearname}_Type{typeid}_{rgname}_{ss}.pkl", "rb") as f:
                    ...
                    leavetime = np.load(f'{INDIR}/{dtname}_LeaveTimePointr2Blk1stDay_type{typeid}_{cyc}_{rgname}_{ss}.npy')
                    ...
                    np.save(f'{OUTDIR}/{dtname}_MiddleEddiesTrackLWAseries_Type{typeid}_{rgname}_{ss}_{cyc}_composites.npy', np.array(TrackAVGLWAlist))
                    np.save(f'{OUTDIR}/{dtname}_MiddleEddiesBlkLWAseries_Type{typeid}_{rgname}_{ss}_{cyc}_composites.npy', np.array(BlkAVGLWAlist))
