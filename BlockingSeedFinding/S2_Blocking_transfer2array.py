...
for dtname in datasets:
    ...
    for eve in eventType: 
        for rgname in regions:
            ...
            if eve == "Seeding":
                lon_min = lon_min - 30 # for seeding, we expand the region 30 degrees westward, to include more potential seeding events that are close to the original region boundary.
            if rgname == 'SP':
                k = 'SH'
                Blklat = lat[0:lat_mid-1] # lat increasing
            else:
                k = 'NH'
                Blklat = lat[lat_mid:len(lat)] # lat increasing
            print(Blklat)
            ...
            for type_idx in range(3):
                ...
                for i in range(len(seasons)):
                    ...
                    blocking_array = np.zeros((len(Date), len(Blklat), len(Blklon)),dtype=np.bool_)
                    blockingID_array = np.full((len(Date), len(Blklat), len(Blklon)),fill_value=-1, dtype=np.int32)
                    ...
                    # save the blocking array: 3d array, [time, lat, lon], bool, mask of block or not; if not: 0/False
        ...
        print(f"{dtname}_{eve}_Type{type_idx+1}_{rgname}_{ss} All events filtered and saved", flush=True)
    ...
    for eve in eventType:
        for type_idx in range(3):
            for i in range(len(seasons)):
                ...
                ss = seasons[i]
                rg1 = np.load(f"{OUT_DIR}/{dtname}_SD_{eve}FlagmaskClusters_Type{type_idx+1}_{regions[0]}_{ss}.npy")
                rg2 = np.load(f"{OUT_DIR}/{dtname}_SD_{eve}FlagmaskClusters_Type{type_idx+1}_{regions[1]}_{ss}.npy")
                rg3 = np.load(f"{OUT_DIR}/{dtname}_SD_{eve}FlagmaskClusters_Type{type_idx+1}_{regions[2]}_{ss}.npy")
                ...
                rgworld = np.concatenate([rg3, rgNH], axis=1) # lat increasing, from -90 to 90, so we put SH in the front 
                ...
                plt.show()
                plt.tight_layout()
                plt.savefig(f'{dtname}_{eve}GlobalFrequency_{blkTypes[type_idx]}_{ss}.png')
                plt.close()
