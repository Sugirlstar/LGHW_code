...
# transfer 6-houly to daily
def sixhourly_to_daily(data):
    T = data.shape[0]
    T4 = T // 4
    data = data[:T4*4]
    data = data.reshape(T4, 4, *data.shape[1:]).mean(axis=1)
    return data
...
for dtname in datasets:
    ...
    lat = np.load(f"{OUT_DIR}/{dtname}_LWA_lat_{yearname}_6hr.npy")
    lon = np.load(f"{OUT_DIR}/{dtname}_LWA_lon_{yearname}_6hr.npy")
    LWA_td = np.load(f"{OUT_DIR}/{dtname}_LWA_td_{yearname}_6hr.npy") 
    LWA_td_A = np.load(f"{OUT_DIR}/{dtname}_LWA_td_A_{yearname}_6hr.npy")
    LWA_td_C = np.load(f"{OUT_DIR}/{dtname}_LWA_td_C_{yearname}_6hr.npy") 
    # daily averaged data:
    LWA_td = sixhourly_to_daily(LWA_td)
    LWA_td_A = sixhourly_to_daily(LWA_td_A)
    LWA_td_C = sixhourly_to_daily(LWA_td_C)

    # latitude mangement:
    # get the NH part
    lat_mid = int(len(lat)/2) + 1 #91
    lat_NH = lat[0:lat_mid-1] #-90~0
    LWA_Z = LWA_td[:,0:lat_mid-1,:] # SH only!
    LWA_Z_A = LWA_td_A[:,0:lat_mid-1,:] # SH only!
    LWA_Z_C = LWA_td_C[:,0:lat_mid-1,:] # SH only!
    print(lat_NH)
    nlon = len(lon)
    nlat = len(lat)
    nlat_NH =len(lat_NH)
    ...
    # time management:
    # get the first and last year name from the yearname string
    ...
    def BKSDIdentifyFun(Duration, LonWidth, ThreshPercentile, stationary=True):
        ...
        print(B_freq[:,1]-B_freq[:,0])
        print(B_freq[:,-1]-B_freq[:,-2])
    ...
    outdir = Path(OUT_DIR)
    save_dict = {
        f"{dtname}_SD_Blocking_peaking_date_daily_SH": Blocking_peaking_date,
        f"{dtname}_SD_Blocking_peaking_lon_daily_SH": Blocking_peaking_lon,
        f"{dtname}_SD_Blocking_peaking_lat_daily_SH": Blocking_peaking_lat,
        f"{dtname}_SD_BlockingTotal_date_SH": BlockingTotal_date,
        f"{dtname}_SD_BlockingTotal_label_SH": BlockingTotal_label,
        f"{dtname}_SD_BlockingTypeI_SH": BlockingTypeI,
        f"{dtname}_SD_Blocking_diversity_date_daily_SH": Blocking_diversity_date,
        f"{dtname}_SD_Blocking_diversity_label_daily_SH": Blocking_diversity_label,
        f"{dtname}_SD_Blocking_diversity_peaking_date_daily_SH": Blocking_diversity_peaking_date,
        f"{dtname}_SD_Blocking_diversity_peaking_lon_daily_SH": Blocking_diversity_peaking_lon,
        f"{dtname}_SD_Blocking_diversity_peaking_lat_daily_SH": Blocking_diversity_peaking_lat,
        f"{dtname}_SD_Blocking_freq_SH": B_freq
    }

    for name, obj in save_dict.items():
    ...
    #%% Save results
    save_dict = {
        f"{dtname}_SD_Seeding_peaking_date_daily_SH": Seeding_peaking_date,
        f"{dtname}_SD_Seeding_peaking_lon_daily_SH": Seeding_peaking_lon,
        f"{dtname}_SD_Seeding_peaking_lat_daily_SH": Seeding_peaking_lat,
        f"{dtname}_SD_SeedingTotal_date_SH": SeedingTotal_date,
        f"{dtname}_SD_SeedingTotal_label_SH": SeedingTotal_label,
        f"{dtname}_SD_SeedingTypeI_SH": SeedingTypeI,
        f"{dtname}_SD_Seeding_diversity_date_daily_SH": Seeding_diversity_date,
        f"{dtname}_SD_Seeding_diversity_label_daily_SH": Seeding_diversity_label,
        f"{dtname}_SD_Seeding_diversity_peaking_date_daily_SH": Seeding_diversity_peaking_date,
        f"{dtname}_SD_Seeding_diversity_peaking_lon_daily_SH": Seeding_diversity_peaking_lon,
        f"{dtname}_SD_Seeding_diversity_peaking_lat_daily_SH": Seeding_diversity_peaking_lat,
        f"{dtname}_SD_Seeding_freq_SH": SD_freq
    }
    ...
    plt.ylabel('Latitude')
    plt.savefig(f'{dtname}_SD_blockingFreq_daily_watershed_SH.png')  
    plt.show()
    plt.close()
    ...
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
    plt.savefig(f'{dtname}_SD_seedingFreq_daily_watershed_SH.png')  
    plt.show()
    plt.close()

    # remove to save memory
    del Blocking_peaking_date, Blocking_peaking_lon, Blocking_peaking_lat, B_freq, Blocking_diversity_date, Blocking_diversity_label, BlockingTotal_date, BlockingTotal_label, BlockingTypeI, Blocking_diversity_peaking_date, Blocking_diversity_peaking_lon, Blocking_diversity_peaking_lat
    del Seeding_peaking_date, Seeding_peaking_lon, Seeding_peaking_lat, SD_freq, Seeding_diversity_date, Seeding_diversity_label, SeedingTotal_date, SeedingTotal_label, SeedingTypeI, Seeding_diversity_peaking_date, Seeding_diversity_peaking_lon, Seeding_diversity_peaking_lat
